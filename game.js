...
const enemies = [
    {
        ...
        maxHealth: 100,
        health: 100,
        damaged: false,  // For flash effect when hit
        damageTimer: 0
    },
    {
        x: 1300,
...
function update() {
    ...
    for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        enemies.forEach(enemy => {
            if (checkCollision(bullet, enemy)) {
                ...
                enemy.damaged = true;
                enemy.damageTimer = 10; // Frames the enemy will flash
    ...
    // Update enemy damage flash timer
    enemies.forEach(enemy => {
...
function draw() {
    ...
    enemies.forEach(enemy => {
        // Draw enemy body
        ctx.fillStyle = enemy.damaged ? '#FF9999' : '#00FF00';  // Flash red when hit
...
function createHitEffect(x, y) {
    for (let i = 0; i < 5; i++) {
        ...
        particles.push({
            ...
            color: '#FFD700',  // Gold color for hit sparks
            lifetime: 20
        });
    }
}

// Add death effect function
function createDeathEffect(x, y) {
    for (let i = 0; i < 20; i++) {
        ...
        particles.push({
            ...
            color: '#8B0000',  // Dark red for death effect
            lifetime: 45
        });
    }
} 